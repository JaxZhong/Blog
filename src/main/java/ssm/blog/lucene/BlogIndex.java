...
public class BlogIndex {
	...
	//添加博客索引，把博客id，博客标题，博客内容放入Document，进行中文分词写入，写入博客索引文件
	...
	public List<Blog> searchBlog(String q) throws Exception {
		...
		booleanQuery.add(query1, BooleanClause.Occur.SHOULD);//或关系
		booleanQuery.add(query2, BooleanClause.Occur.SHOULD);//或关系
		
		TopDocs hits = search.search(booleanQuery.build(), 100);
		
		QueryScorer scorer = new QueryScorer(query1);//n
