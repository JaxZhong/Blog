...
public class PageUtil {
	...
	//获取上下篇博客
	public static String getPrevAndNextPageCode(Blog prevOneBlog, Blog nextOneBlog, String projectContent) {
		StringBuffer pageCode = new StringBuffer();
		if(prevOneBlog == null || prevOneBlog.getId() == null) {
			pageCode.append("<p>上一篇：无</P>");
		} else {
			pageCode.append("<p>下一篇：<a href='" + projectContent + "/blog/articles/" + prevOneBlog.getId() + ".html'>" + prevOneBlog.getTitle() + "</a></p>");
		}
		
		if(nextOneBlog == null || nextOneBlog.getId() == null) {
			pageCode.append("<p>下一篇：无</P>");
		} else {
			pageCode.append("<p>上一篇：<a href='" + projectContent + "/blog/articles/" + nextOneBlog.getId() + ".html'>" + nextOneBlog.getTitle() + "</a></p>");
		}
